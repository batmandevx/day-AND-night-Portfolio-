...
export const createScrollCue = (reduceMotion: boolean) => {
    ...
    const updateSatellites = (): void => {
        if (!scrollCueOutline || !scrollCueOutline.isConnected || outlineLength <= 0) {
            return;
        }
    ...
    const measure = (): void => {
        if (!scrollCueOutline || !scrollCueOutline.isConnected) {
            return;
        }
        outlineLength = scrollCueOutline.getTotalLength();
        updateSatellites();
    };
